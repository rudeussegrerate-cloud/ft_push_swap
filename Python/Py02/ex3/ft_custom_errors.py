...
def raiseError(Errorexcept: GardenError, message: str) -> None:
	if (message):
...
class Plant:
	...
	def get_waterqty(self) -> float:
		return self._waterqty

	def set_waterqty(self, value) -> None:
		if (value < 25):
...
if __name__ == "__main__":
	...
	i = 0
	a = ''
	except_list = (GardenError, GardenError, GardenError, GardenError)
	while i < 4:
		try:
			if (i < 3):
